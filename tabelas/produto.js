...
const validate_nome = (nome_produto) => {
  if (nome_produto === undefined) return true;

  if (nome_produto == null) return "nome_produto nao pode ser NULL";

  if (!isString(nome_produto)) return "nome_produto precisa ser uma string";
...
const validate_codigo = (cod_produto) => {
  if (cod_produto === undefined) return true;

  if (cod_produto === null) return "cod_produto nao pode ser NULL";
...
const validate_categoria = (categoria) => {
  if (categoria === undefined) return true;

  if (categoria === null) return "categoria nao pode ser NULL";

  if (!isString(categoria)) return "categoria precisa ser uma string";

  if (categoria.length === 0) return "categoria nao pode ser uma string vazia";
...
const validate_preco = (preco) => {
  if (preco === undefined) return true;

  if (preco === null) return "preco nao pode ser NULL";

  if (isNaN(preco)) return "preco precisa ser um numero";

  if (preco <= 0) return "preco precisa ser maior que zero";

  return true;
};

const validate_estoque = (qtd_estoque) => {
  if (qtd_estoque === undefined) return true;

  if (qtd_estoque === null) return "qtd_estoque nao pode ser NULL";

  if (!Number.isInteger(qtd_estoque))
    return "qtd_estoque precisa ser um inteiro";

  if (qtd_estoque < 0) return "qtd_estoque nao pode ser negativo";

  return true;
};

const validate_mari = (feito_em_Mari) => {
  if (feito_em_Mari === undefined) return true;

  if (feito_em_Mari === null) return "feito_em_Mari nao pode ser NULL";
    ...
    const to_update = Object.assign(
      {},
      {
        nome_produto: nome_produto
          ? connection.escape(nome_produto)
          : undefined,
